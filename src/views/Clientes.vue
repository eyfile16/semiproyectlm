<template>
  <div class="table-container">
    ...
    <div class="welcome-text">
      ...
      <p>En esta sección podrás ver todos los proveedores registrados. Puedes actualizar la lista de proveedores en cualquier momento haciendo clic en el botón "Actualizar". Aquí podrás obtener información relevante sobre cada uno de nuestros proveedores.</p>
    </div>
    ...
    >
      <template v-slot:top-right>
        <q-btn label="Actualizar" @click="fetchProveedores" color="primary" class="update-btn" />
      </template>
    </q-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    // Método para obtener los proveedores desde la API
    fetchProveedores() {
      axios.get(`http://localhost:3000/api/proveedores/${this.selectedCategory}`)
        .then(response => {
  ...
  mounted() {
    this.fetchProveedores(); // Llamada a la API cuando el componente se monta
  }
};
</script>
