<template>
  <div class="table-container">
    ...
    <div class="welcome-text">
      <h1>Bienvenido a Artículos</h1>
      <p>Explora nuestra amplia variedad de artículos disponibles. Aquí podrás ver todos los artículos que tenemos en inventario. Puedes actualizar la lista de artículos en cualquier momento haciendo clic en el botón "Actualizar".</p>
    </div>
    ...
    >
      <template v-slot:top-right>
        <q-btn label="Actualizar" @click="fetchArticulos" color="primary" class="update-btn" />
      </template>
    </q-table>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetchArticulos() {
      axios.get(`http://localhost:3000/api/articulos/${this.selectedCategory}`)
        .then(response => {
  ...
  mounted() {
    this.fetchArticulos(); // Llamada a la API cuando el componente se monta
  }
};
</script>
